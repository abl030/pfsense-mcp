...
- `pfsense_update_services_dns_forwarder_host_override_alias`
...
9. **Update** using `pfsense_update_services_dns_forwarder_host_override_alias` with `confirm=True` — set `domain` to `updated.alias.example.com`
10. **Get** again using `pfsense_get_services_dns_forwarder_host_override_alias` — verify `domain` was updated
11. **Check apply status** using `pfsense_get_services_dns_forwarder_apply_status`
12. **Apply changes** using `pfsense_services_dns_forwarder_apply` with `confirm=True`
