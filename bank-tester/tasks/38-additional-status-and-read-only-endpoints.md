...
- `pfsense_get_status_i_psec_child_sa`
- `pfsense_list_status_i_psec_child_s_as`
- `pfsense_list_status_logs_open_vpn`
...
**Steps**:
1. **Read** using `pfsense_get_status_i_psec_child_sa` (Get singular IPsec child SA)
2. **Read** using `pfsense_list_status_i_psec_child_s_as` (List IPsec child SAs)
3. **Read** using `pfsense_list_status_logs_open_vpn` (OpenVPN logs — use limit parameter)
...
11. **Read** using `pfsense_list_system_cr_ls` (List all CRLs (plural))
