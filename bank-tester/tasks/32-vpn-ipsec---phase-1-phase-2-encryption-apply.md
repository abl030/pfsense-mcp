...
- `pfsense_update_vpni_psec_phase1_encryption`
...
- `pfsense_update_vpni_psec_phase2_encryption`
...
9. **Update** using `pfsense_update_vpni_psec_phase1_encryption` with `confirm=True` — set `encryption_algorithm_keylen` to `128`
10. **Get** again using `pfsense_get_vpni_psec_phase1_encryption` — verify `encryption_algorithm_keylen` was updated
11. **Create** using `pfsense_create_vpni_psec_phase2` with `confirm=True` (inject `ikeid` from parent's `ikeid` field):
    - `mode`: `tunnel`
    ...
    - `protocol`: `esp`
    - `descr`: `bt_sys32_p2`
12. **List** using `pfsense_list_vpni_psec_phase2s` — verify the created resource appears
13. **Get** using `pfsense_get_vpni_psec_phase2` with the ID from the create response
14. **Update** using `pfsense_update_vpni_psec_phase2` with `confirm=True` — set `descr` to `Updated P2`
15. **Get** again using `pfsense_get_vpni_psec_phase2` — verify `descr` was updated
16. **Create** using `pfsense_create_vpni_psec_phase2_encryption` with `confirm=True` (use the `parent_id` from the parent resource created earlier):
    - `encryption_algorithm_name`: `aes`
    - `encryption_algorithm_keylen`: `256`
    - `hash_algorithm`: `sha256`
17. **List** using `pfsense_list_vpni_psec_phase2_encryptions` — verify the created resource appears
18. **Get** using `pfsense_get_vpni_psec_phase2_encryption` with the ID from the create response
19. **Update** using `pfsense_update_vpni_psec_phase2_encryption` with `confirm=True` — set `hash_algorithm` to `sha512`
20. **Get** again using `pfsense_get_vpni_psec_phase2_encryption` — verify `hash_algorithm` was updated
21. **Check apply status** using `pfsense_get_vpni_psec_apply_status`
22. **Apply changes** using `pfsense_vpni_psec_apply` with `confirm=True`
