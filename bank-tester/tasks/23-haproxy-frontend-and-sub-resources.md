...
8. **Get** using `pfsense_get_services_ha_proxy_frontend_acl` with the ID from the create response
9. **Update** using `pfsense_update_services_ha_proxy_frontend_acl` with `confirm=True` — set `value` to `updated.example.com`
10. **Get** again using `pfsense_get_services_ha_proxy_frontend_acl` — verify `value` was updated
11. **Create** using `pfsense_create_services_ha_proxy_frontend_action` with `confirm=True` (use the `parent_id` from the parent resource created earlier):
    - `action`: `http-request_lua`
    - `name`: `bt_sys23_fact`
    - `acl`: `bt_sys23_facl_sib`
12. **Get** using `pfsense_get_services_ha_proxy_frontend_action` with the ID from the create response
13. **Create** using `pfsense_create_services_ha_proxy_frontend_address` with `confirm=True` (use the `parent_id` from the parent resource created earlier):
    - `extaddr`: `custom`
    - `extaddr_custom`: `10.99.99.80:80`
14. **Get** using `pfsense_get_services_ha_proxy_frontend_address` with the ID from the create response
15. **Create** using `pfsense_create_services_ha_proxy_frontend_certificate` with `confirm=True` (use the `parent_id` from the parent resource created earlier):
(no parameters needed)
16. **Get** using `pfsense_get_services_ha_proxy_frontend_certificate` with the ID from the create response
17. **Create** using `pfsense_create_services_ha_proxy_frontend_error_file` with `confirm=True` (use the `parent_id` from the parent resource created earlier):
    - `errorcode`: `503`
    - `errorfile`: `bt_sys23_file`
18. **Get** using `pfsense_get_services_ha_proxy_frontend_error_file` with the ID from the create response
19. **Check apply status** using `pfsense_get_services_ha_proxy_apply_status`
20. **Apply changes** using `pfsense_services_ha_proxy_apply` with `confirm=True`
